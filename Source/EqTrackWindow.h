...
class EqTrackWindow : public DocumentWindow,
    public Slider::Listener
{
public:
    EqTrackWindow(String name, int idx) : DocumentWindow(name, Colours::grey, DocumentWindow::allButtons)
    {
        index = idx;
    ...
    void sliderValueChanged(Slider* slider) override {
        if (slider == &low) {
            te::getAudioTracks(*edit)[index]->getAllPlugins()[2]->getAutomatableParameter(1)->setParameter(slider->getValue(), sendNotification);
        }
        else if (slider == &low_mid) {
            te::getAudioTracks(*edit)[index]->getAllPlugins()[2]->getAutomatableParameter(10)->setParameter(slider->getValue(), sendNotification);
        }
        else if (slider == &high_mid) {
            te::getAudioTracks(*edit)[index]->getAllPlugins()[2]->getAutomatableParameter(7)->setParameter(slider->getValue(), sendNotification);
        }
        else if (slider == &high) {
            te::getAudioTracks(*edit)[index]->getAllPlugins()[2]->getAutomatableParameter(4)->setParameter(slider->getValue(), sendNotification);
        }
    }
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(EqTrackWindow)
};